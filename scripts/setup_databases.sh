...
for usb_mount in "/Volumes/DeCodeDNA_DB" "/media/$USER/DeCodeDNA_DB" "/mnt/DeCodeDNA_DB"; do
  if [[ -d "$usb_mount/databases/kraken2_db" ]]; then
    ...
    echo "🔧 To use these databases, run:"
    echo "   source scripts/setup_databases.sh"
    echo ""
