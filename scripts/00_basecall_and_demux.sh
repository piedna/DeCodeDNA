...
# Model to actually run (FAST model, for speed demo in classroom)
...
if [[ -d "$FAST5_PATH" ]] && compgen -G "$FAST5_PATH"/*.fast5 >/dev/null; then
  ...
  echo ""
  echo "     Skipping conversion - using existing POD5 files for basecalling demo"
...
echo ""
echo "  APPROACH 1: GPU/Metal Basecalling (if available)"
echo "─────────────────────────────────────────────────"
...
if [[ "$GPU_AVAILABLE" == "true" ]]; then
  ...
  echo "    Skipping GPU basecalling (no acceleration detected)"
  GPU_DURATION=0
fi
...
if [[ -f "$GPU_OUTPUT" && -f "$CPU_OUTPUT" && "$GPU_DURATION" -gt 0 ]]; then
  echo "     GPU/Metal basecalling: ${GPU_DURATION}s"
