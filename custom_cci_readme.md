...
```bash
# Navigate to Custom CCI directory
cd workflows/custom_cci_barcodes/

# Step 1: Quality filter raw data (3-5 min)
bash ../../scripts/00_quality_filter_predemux.sh \
    00_raw_data/test_fhl_customcci.fastq \
    01_filtered/quality_filtered.fastq

# Step 2: Manual demultiplexing with ONTbarcoder2.3 GUI (5-10 min)
...
**Total workflow time: 25-35 minutes**

---

## 🆚 Comparison: Custom CCI vs ONT Native

| Aspect | Custom CCI Workflow | ONT Native Workflow |
|--------|-------------------|-------------------|
| **Setup complexity** | High (manual demux) | Low (ready files) |
| **Time required** | 25-35 minutes | 15-20 minutes |
| **Flexibility** | High (custom primers) | Medium (standard kits) |
| **Skills learned** | Complete wet lab → analysis | Focus on bioinformatics |
| **Best for** | Research applications | Classroom/standard analysis |
| **Prerequisites** | ONTbarcoder2.3 GUI | None (ready data) |

---

## 🔬 Scientific Applications

### When Custom CCI is Essential
- **Novel taxonomic groups**: Primers not in standard kits
- **Ancient DNA**: Modified protocols for degraded samples
- **Multi-gene studies**: Different amplicons in same library
- **Method development**: Testing new primer combinations
- **Environmental monitoring**: Taxon-specific primer sets

### Research Examples
```bash
# Example 1: Fish community with custom 12S primers
sample_name,forward_tag,reverse_tag,forward_primer,reverse_primer
lake_A_summer,ACGTACGTACGT,ACGTACGTACGT,GTCGGTAAAACTCGTGCCAGC,CATAGTGGGGTATCTAATCCCAGTTTG
lake_A_winter,TGCATGCATGCA,TGCATGCATGCA,GTCGGTAAAACTCGTGCCAGC,CATAGTGGGGTATCTAATCCCAGTTTG

# Example 2: Multi-amplicon study (12S + COI)
sample_name,forward_tag,reverse_tag,forward_primer,reverse_primer
site1_12S,AAACCGTGTTTT,AAACCGTGTTTT,GTCGGTAAAACTCGTGCCAGC,CATAGTGGGGTATCTAATCCCAGTTTG
site1_COI,AAACCGTGTTTT,AAACCGTGTTTT,GGTCAACAAATCATAAAGATATTGG,TAAACTTCAGGGTGACCAAAAAATCA
```
...
### Pre-Sequencing Checklist
- ✅ **Barcode design**: Unique sequences, no cross-reactivity
- ✅ **Primer validation**: Tested on target taxa
- ✅ **Library preparation**: Proper indexing and pooling
- ✅ **Demux sheet**: Accurate sample-barcode mapping

### Post-Sequencing Workflow
1. **Basecalling**: MinKNOW or Dorado (HAC/SUP models)
2. **Quality assessment**: Check overall sequence quality
3. **Pre-demux filtering**: Remove low-quality reads early
4. **Manual demultiplexing**: ONTbarcoder2.3 with custom sheet
5. **Format correction**: EFPQ base conversion
6. **Core analysis**: Standard DeCodeDNA pipeline

### Quality Control Checkpoints
- **Raw data**: >1M reads, Q-score distribution
- **Post-filtering**: 70-90% retention typical
- **Demultiplexing**: >80% assignment rate
- **Species identification**: Cross-validate with morphology

---

## 🧪 Laboratory Integration

### Wet Lab → Bioinformatics Bridge
This workflow teaches the complete eDNA pipeline:

**Wet Lab Skills:**
- Primer design and testing
- Barcode scheme development  
- Library preparation optimization
- Quality control validation

**Bioinformatics Skills:**
- Data quality assessment
- Manual demultiplexing strategies
- Pipeline parameter optimization
- Results interpretation and validation

### Integration with Course Curriculum
- **Week 1**: Primer design and barcode planning
- **Week 2**: Library preparation and sequencing
- **Week 3**: Custom CCI workflow execution  
- **Week 4**: Results interpretation and validation
...
```bash
# Good demultiplexing results:
# - Assignment rate: >80%
# - Sample balance: CV < 50% across samples  
# - Cross-contamination: <5% between samples

# Check demultiplexing stats
wc -l 02_demultiplexed/*.fa
# Should show similar line counts across samples

# Validate base conversion
grep -c "[EFQP]" 02_demultiplexed/*.fa
# Should return 0 after EFPQ conversion
```

### Troubleshooting Failed Demultiplexing
```bash
# Check if barcodes are present in raw data
grep -c "YOUR_BARCODE_SEQUENCE" 01_filtered/quality_filtered.fastq

# Test different ONTbarcoder settings:
# - Increase mismatch tolerance
# - Adjust minimum barcode score
# - Check primer orientation (forward/reverse)

# Validate demux sheet format
head -5 demux_config/demux_sheet.csv
# Ensure: no spaces, exact column names, proper CSV format
```
...
### ONTbarcoder2.3 Documentation
- **GitHub**: https://github.com/asrivathsan/ONTbarcoder
- **Manual**: Detailed parameter explanations
- **Troubleshooting**: Common error solutions

### Primer Design Tools  
- **Primer3**: General primer design
- **ecoPrimers**: Environmental DNA primer design
- **NCBI Primer-BLAST**: Specificity checking

### Barcode Design Guidelines
- **Minimum distance**: >3 bp between barcodes
- **GC content**: 40-60% for stability
- **Length**: 8-12 bp optimal for ONT
- **Avoid homopolymers**: No >3 consecutive identical bases
...
After completing this workflow, students should be able to:

1. **Design custom barcode schemes** for multiplexed sequencing
2. **Execute manual demultiplexing** using specialized software
3. **Troubleshoot common demultiplexing issues** 
4. **Integrate wet lab and bioinformatics** workflows
5. **Optimize parameters** for specific research applications

### Assessment Rubric
- **Technical execution**: Successful demultiplexing (>80% assignment)
- **Parameter optimization**: Appropriate quality filtering
- **Results interpretation**: Biological meaning of species identifications
- **Troubleshooting**: Problem-solving for failed steps

### Extension Exercises
1. **Compare primer sets**: Design alternative primers for same taxa
2. **Optimize barcode schemes**: Test different barcode lengths/designs
3. **Cross-validate results**: Compare with morphological identifications
4. **Scale analysis**: Apply to real research samples

---

**🎯 This workflow provides complete control over the demultiplexing process while maintaining the power of the DeCodeDNA analysis pipeline - perfect for advanced students and research applications!**